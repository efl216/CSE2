...
public class Root {
    ...
    public static void main(String[] args) {
        ...
        double number = myScanner.nextInt(); //accepts the user input 
        
        double guess= number/3;
        double root1=(2*guess*guess*guess+number)/(3*guess*guess);
        double root2=(2*root1*root1*root1+number)/(3*root1*root1);
        double root3=(2*root2*root2*root2+number)/(3*root2*root2);
        double root4=(2*root3*root3*root3+number)/(3*root3*root3);
        double cube=root4*root4*root4;
